...
### e1.py 핵심 코드
```python
img_bgr = cv.imread(str(image_path))  # 입력 이미지를 BGR 형식으로 로드
img_gray = cv.cvtColor(img_bgr, cv.COLOR_BGR2GRAY)  # BGR -> grayscale(1채널)
img_gray_bgr = cv.cvtColor(img_gray, cv.COLOR_GRAY2BGR)  # hstack을 위해 3채널로 복원
merged = np.hstack([img_bgr, img_gray_bgr])  # 원본과 grayscale 결과를 좌우 결합

cv.imshow("Original | Grayscale", merged)  # 결합 결과를 화면에 표시
...
def on_mouse(self, event: int, x: int, y: int, flags: int, param) -> None:
    if event == cv.EVENT_LBUTTONDOWN:
        self._draw(x, y, (255, 0, 0))  # 좌클릭: 파란색(BGR) 점 그리기
        return
    if event == cv.EVENT_RBUTTONDOWN:
        self._draw(x, y, (0, 0, 255))  # 우클릭: 빨간색(BGR) 점 그리기
        return
    if event == cv.EVENT_MOUSEMOVE:
        if flags & cv.EVENT_FLAG_LBUTTON:  # 좌버튼 드래그 중이면
            self._draw(x, y, (255, 0, 0))  # 파란색 연속 그리기
        elif flags & cv.EVENT_FLAG_RBUTTON:  # 우버튼 드래그 중이면
            self._draw(x, y, (0, 0, 255))  # 빨간색 연속 그리기
```

```python
if key in (ord("+"), ord("=")):
    painter.brush_size += 1  # 붓 크기 1 증가
    painter._clamp_brush()  # 요구사항 범위(1~15)로 제한

if key in (ord("-"), ord("_")):
    painter.brush_size -= 1  # 붓 크기 1 감소
    painter._clamp_brush()  # 요구사항 범위(1~15)로 제한
```
...
```python
if event == cv.EVENT_LBUTTONDOWN:
    self.selecting = True  # ROI 드래그 시작 상태
    self.start_xy = (x, y)  # 시작 좌표 기록
    self.end_xy = (x, y)  # 초기 끝 좌표 설정

if event == cv.EVENT_MOUSEMOVE and self.selecting:
    self.end_xy = (x, y)  # 드래그 중 끝 좌표 갱신
    self._update_display_rectangle()  # 현재 선택 박스를 실시간 표시

if event == cv.EVENT_LBUTTONUP and self.selecting:
    self.selecting = False  # 드래그 종료
    self.end_xy = (x, y)  # 최종 끝 좌표 기록
    self._finalize_roi()  # 좌표 정규화 후 ROI 추출/표시
```

```python
x_min, x_max = sorted([x0, x1])  # x축 min/max 정렬 (드래그 방향 무관)
y_min, y_max = sorted([y0, y1])  # y축 min/max 정렬 (드래그 방향 무관)
self.roi = self.image[y_min:y_max, x_min:x_max].copy()  # numpy 슬라이싱으로 ROI 추출
cv.imshow("ROI", self.roi)  # 추출한 ROI를 별도 창에 출력
```
